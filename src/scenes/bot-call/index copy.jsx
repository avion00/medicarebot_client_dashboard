...
import PlayCircleFilledIcon from "@mui/icons-material/PlayCircleFilled"; // Import Play Icon
...
const CallingPage = () => {
  ...
  useEffect(() => {
    if (!isOnHold) {
      ...
      clearInterval(timerRef.current); // Stop the timer when call is on hold
    }

    return () => clearInterval(timerRef.current); // Cleanup on unmount
  }, [isOnHold]); // Depend only on isOnHold
  ...
  // Toggle Hold (Pause/Resume)
  const handlePause = () => {
    setIsOnHold((prev) => !prev); // Toggle hold state
  };
