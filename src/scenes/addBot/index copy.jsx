...
const AddBot = () => {
  ...
  const handleFormSubmit = async (values, e) => {
    ...
    try {
      ...
      // Append text fields compulsory details
      ...
      formData.append("language_support", values.languageSupport);
      formData.append("pretrained_template", values.preTrainedTemplate);
      formData.append("expectation", values.ExpectedOutcome);
      formData.append("knowlege_base_file", values.uploadKnowledgeBase);

      // Append the image and file
      if (imageOne) {
        const AvatarImage = dataURItoBlob(imageOne);
        formData.append("avatar", AvatarImage);
      }

      // Append text fields optional details
      formData.append(
        "roleAndPurposeExplanation",
        values.roleAndPurposeExplanation
      );
      formData.append("response_time", values.responseTime);
      formData.append("expected_achieve", values.ExpectedAchieve);
      formData.append("attact_documents", values.attachDocuments);
      formData.append(
        "upload_optional_docuement",
        values.uploadOptionalDocument
      );
      formData.append("api_key", values.apiKey);
      formData.append("callback_url", values.callbackURL);
      formData.append("start_url", values.startURL);
      formData.append("depth", values.depth);
      formData.append("focus_keywords", values.focusKeywords);

      if (imageTwo) {
        const IconImage = dataURItoBlob(imageTwo);
        formData.append("icon_image", IconImage);
      }

      if (imageThree) {
        const IconImageOptional = dataURItoBlob(imageThree);
        formData.append("icon_image_optional", IconImageOptional);
      }
      ...
      if (response.data.success) {
        ...
        setNotificationMessage(
          "Congratulation, You Created New Bot, lets integrate..."
        );
        setShowNotification(true);
        // setTimeout(() => navigate("/login"), 2000);
      } else {
        throw new Error(
          response.data.message || "Sorry!, Bot Creation failed."
        );
      }
    } catch (error) {
  ...
  const checkoutSchema = yup.object().shape({
    ...
    // imageOne: yup.mixed().required("Image One is required"), // For file upload, use `mixed` type
    imageOne: yup
      .mixed()
                  ...
                  >
                    ...
                    >
                      ...
                      <input
                        ...
                        accept="image/*"
                        style={{ display: "none" }}
                        onChange={(e) => handleFileOneChange(e, handleChange)}
                      />
                    </Box>
                  ...
                  >
                    ...
                    >
                      ...
                      <input
                        ...
                        accept="image/*"
                        style={{ display: "none" }}
                        onChange={(e) => handleFileTwoChange(e, handleChange)}
                      />
                    </Box>
                    <Typography mb="2em" variant="h6">
                      Upload Website Icon
                  ...
                  >
                    ...
                    >
                      ...
                      <input
                        ...
                        accept="image/*"
                        style={{ display: "none" }}
                        onChange={(e) => handleFileThreeChange(e, handleChange)}
                      />
                    </Box>
                    <Box
                      sx={{
